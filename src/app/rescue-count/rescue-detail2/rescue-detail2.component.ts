...
@Component({
  selector: 'app-rescue-detail2',
  templateUrl: './rescue-detail2.component.html',
  styleUrls: ['./rescue-detail2.component.css']
})
export class RescueDetailComponent2 implements OnInit {
  rescues: Rescue[];
  rescue: Rescue;
  ...
  ngOnInit() {
    ...
    this.route.params
      .switchMap((params: Params) => this.rescueCountService.getMenuDatas(1,10,params['rescueCenterId']))
      .subscribe(res =>{
        layer.close(index);
        if(res['code'] == 0){
          if(res['data']['list']){
  ...
  changePage(page,index) {
    ...
    this.rescueCountService.getMenuDatas(index,10,this.curId).then( res => {
      if(res['code'] == 0){
        this.rescues = res['data']['list'];
        this.curPage = res['data']['pageNum'];
        this.setPagingArr();
